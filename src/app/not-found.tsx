...
import { ArrowLeft, Compass, Home, Layers3, Mail } from "lucide-react";
          ...
          <div className="grid gap-10 md:grid-cols-[220px_minmax(0,1fr)] md:items-center">
            ...
            <div className="space-y-5 text-center md:text-left">
              ...
              <p className="max-w-xl text-base leading-relaxed text-(--muted)">
                ...
                moved, or was never part of this portfolio. The good news is the
                useful parts are still one click away.
              </p>
              ...
              <div className="flex flex-col items-center gap-3 pt-2 sm:flex-row md:items-start">
                ...
                <Link
                  href="/"
                  className="inline-flex items-center gap-2 rounded-full border border-(--border) px-5 py-2.5 text-sm font-medium text-(--muted) transition hover:border-(--foreground)/20 hover:text-(--foreground)"
                >
                ...
                <div className="flex flex-wrap justify-center gap-3 md:justify-start">
                  {quickLinks.map(({ href, label, icon: Icon }) => (
                    <Link
                      key={href}
                      href={href}
                      className="inline-flex items-center gap-2 rounded-full border border-(--border) bg-(--background) px-4 py-2 text-sm font-medium text-(--muted) transition hover:border-(--accent)/30 hover:text-(--accent)"
                    >
